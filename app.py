...
if page == "Dashboard":
    ...
    if casos.empty:
        ...
        with st.form("update_case"):
            ...
            proxima = st.date_input("Próxima revisión", value=pd.to_datetime(casos.loc[idx, "proxima_revision"], errors="coerce").date() if pd.notna(pd.to_datetime(casos.loc[idx, "proxima_revision"], errors="coerce")) else date.today())
